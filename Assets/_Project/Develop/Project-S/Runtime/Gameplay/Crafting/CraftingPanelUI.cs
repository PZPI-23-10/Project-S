    ...
    public class CraftingPanelUI : MonoBehaviour
    {
        ...
        private List<CraftingRecipeData> GetContextRecipes()
        {
            if (IsStationContext(_context)
                && _activeStation != null
                && _activeStation.AvailableRecipes != null
                && _activeStation.AvailableRecipes.Count > 0)
            {
                return _activeStation.AvailableRecipes
                    .Where(x => x != null && x.Context == _context && _activeStation.AllowsRecipe(x))
                    .Distinct()
